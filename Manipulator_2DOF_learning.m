...
for k = 1:length(t)
    ...
    % Controller learning
    nominal(:, k) = control.get_control_inverse_full(qd(:, k), qdp(:, k), qdpp(:, k));
    l(:, k) = control.learning_control(qd(:, k));
    u(:, k) = nominal(:, k) + l(:, k); 
...
save("Data_Learning.mat", "t", "q", "qd", "qdp", "qe", "qep", "u", "T_extern", "l","L1", "L2")